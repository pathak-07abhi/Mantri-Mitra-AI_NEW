 ...
 *   ✅ Constituency Voter/Contact Database (new page)
...
/* ═══════════════════════════════════════════════════════════════
   PAGE: DASHBOARD
═══════════════════════════════════════════════════════════════ */
function Dashboard({ issues, meetings, docs, speeches, setPage, voters=[], rtis=[], isMobile=false }) {
    ...
    <div style={{ paddingBottom: isMobile ? "80px" : "0" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"repeat(3,1fr)", gap:"8px", marginBottom:"12px" }}>
        {[
          ...
          { label:"Voter Contacts", value:voters.length, sub:"In database", accent:"#138808", icon:"👥", page:"voters" },
      ...
      {isMobile && (
        <div style={{ display:"flex", gap:"8px", overflowX:"auto", marginBottom:"12px", paddingBottom:"4px" }}>
          {[
            {icon:"👥",label:"Voter DB",page:"voters",color:"#138808"},
          ...
          <div style={card()}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))", gap:"8px" }}>
              {[
                ...
                {page:"voters",   icon:"👥",label:"Voter DB",color:"#138808"},
...
function getTheme(dark) {
  return dark ? {
    ...
    bg:       "#EAF1FB",
    shellBg:  "radial-gradient(circle at top, #f8fbff 0%, #e9f1ff 38%, #d7e1f2 100%)",
    card:     "rgba(255, 255, 255, 0.60)",
    border:   "rgba(119, 145, 184, 0.20)",
    text:     "#0F172A",
    muted:    "#42566F",
    inp:      "rgba(255,255,255,.58)",
    inpBorder:"rgba(119, 145, 184, 0.24)",
    subBar:   "rgba(255,255,255,.56)",
    subBarBorder:"rgba(119, 145, 184, 0.18)",
    ...
    <div style={{ minHeight:"100vh", width:"100%", display:"flex", flexDirection:"column", background:"radial-gradient(circle at top, #122345 0%, #0D1730 44%, #070C17 100%)", fontFamily:F2, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", position:"relative", zIndex:2 }}>
        <div style={{ width:"100%", maxWidth:"480px" }}>
          ...
          <div style={{ background:"rgba(255,255,255,.08)", border:"1px solid rgba(255,255,255,.16)", borderRadius:"18px", overflow:"hidden", backdropFilter:"blur(24px) saturate(150%)", WebkitBackdropFilter:"blur(24px) saturate(150%)", boxShadow:"0 28px 80px rgba(0,0,0,.42)" }}>
            {/* Tab switcher */}
...
function NotificationCenter({ notifications, setNotifications, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", zIndex:300, display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={onClose}>
      <div style={{ background:"var(--t-card,#fff)", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:"480px", maxHeight:"85vh", display:"flex", flexDirection:"column", boxShadow:"0 -8px 40px rgba(0,0,0,.25)" }} onClick={e=>e.stopPropagation()}>
        {/* Handle */}
        <div style={{ width:"36px", height:"4px", background:"rgba(0,0,0,.15)", borderRadius:"2px", margin:"12px auto 0" }}/>
        {/* Header */}
...
/* ═══════════════════════════════════════════════════════════════
   NEW PAGE: VOTER / CONTACT DATABASE
═══════════════════════════════════════════════════════════════ */
...
export default function App() {
  ...
  const NAV_LABELS = {
    ...
    analytics:"Analytics & Insights", voters:"Voter Database", rti:"RTI Tracker", settings:"Settings"
  };
  const NAV_ITEMS = [
  ...
  const renderPage = () => {
    const m = isMobile;
    switch(page) {
      case "dashboard": return <Dashboard issues={issues} meetings={meetings} docs={docs} speeches={speeches} setPage={setPage} voters={voters} rtis={rtis} T={T} dark={dark} isMobile={m}/>;
      ...
      case "voters":    return <VoterDB voters={voters} setVoters={setVoters} isMobile={m}/>;
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", width:"100%", overflow:"hidden", background:T.shellBg || T.bg, fontFamily:FONT_SANS, color:T.text, transition:"background .3s, color .3s", position:"relative" }}>
      <style>{`
        ...
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
        ...
        .premium-panel{background:linear-gradient(180deg, rgba(255,255,255,.10), rgba(255,255,255,.03));border:1px solid rgba(255,255,255,.14);box-shadow:0 18px 40px rgba(2,8,23,.20), inset 0 1px 0 rgba(255,255,255,.10);backdrop-filter:blur(22px) saturate(155%);-webkit-backdrop-filter:blur(22px) saturate(155%)}
        ...
        .desktop-nav-btn{padding:10px 14px;cursor:pointer;font-family:'Noto Sans', Arial, sans-serif;color:rgba(255,255,255,.68);background:transparent;border:1px solid transparent;border-radius:14px;display:flex;align-items:center;gap:8px;flex-shrink:0;transition:all .18s;white-space:nowrap;position:relative}
        .desktop-nav-btn:hover{background:rgba(255,255,255,.08);color:#fff;border-color:rgba(255,255,255,.10);transform:translateY(-1px)}
        .desktop-nav-btn.active{color:#fff;background:linear-gradient(180deg, rgba(77,163,255,.22), rgba(77,163,255,.10));border-color:rgba(77,163,255,.34);box-shadow:0 12px 26px rgba(77,163,255,.18), inset 0 1px 0 rgba(255,255,255,.12)}
        ...
        .mobile-nav-btn.active{background:linear-gradient(180deg, rgba(77,163,255,.16), rgba(77,163,255,.06));color:#fff;border-top:2px solid rgba(77,163,255,.9)}
      ...
      {/* ══ GOV HEADER ══ */}
      <div className="app-shell premium-panel premium-bar" style={{ background:"linear-gradient(180deg, rgba(5,10,20,.34), rgba(5,10,20,.10))", flexShrink:0 }}>
        {/* ── Top utility bar ── */}
        <div style={{ background:"linear-gradient(90deg, rgba(15,52,96,.86), rgba(18,39,75,.72))", padding:"8px 14px", display:"flex", alignItems:"center", gap:"10px", borderBottom:"1px solid rgba(255,255,255,.08)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"5px", flexShrink:0 }}>
            <div style={{ width:"7px", height:"7px", borderRadius:"50%", background:pulse?"#4ADE80":"#6B7280", boxShadow:pulse?"0 0 6px #4ADE80":"none", transition:"all .5s", position:"relative" }}/>
            <span style={{ fontSize:"11px", color:"rgba(255,255,255,.85)" }}>AI Active</span>
          </div>
          {!isMobile && <><span style={{ fontSize:"11px", color:"rgba(255,255,255,.4)" }}>|</span><span style={{ fontSize:"11px", color:"rgba(255,255,255,.7)" }}>{getGovLabel(settings)} · MPA</span></>}
          <div style={{ flex:1 }}/>
          {!isMobile && <span style={{ fontSize:"11px", color:"rgba(255,255,255,.8)", fontFamily:"monospace" }}>
          ...
          {/* Notification Bell */}
          <button onClick={()=>setShowNotifs(true)} style={{ position:"relative", display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(255,255,255,.10)", border:"1px solid rgba(255,255,255,.18)", borderRadius:"12px", padding:"6px 11px", cursor:"pointer", color:"#fff", gap:"4px", boxShadow:"inset 0 1px 0 rgba(255,255,255,.08)" }}>
            <span style={{ fontSize:"14px" }}>🔔</span>
          ...
          {/* Dark toggle */}
          <button onClick={()=>setDark(d=>!d)} style={{ display:"flex", alignItems:"center", gap:"4px", cursor:"pointer", background:"rgba(255,255,255,.10)", border:"1px solid rgba(255,255,255,.18)", borderRadius:"12px", padding:"6px 11px", fontSize:"11px", color:"rgba(255,255,255,.92)", fontWeight:"600", flexShrink:0, boxShadow:"inset 0 1px 0 rgba(255,255,255,.08)" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"6px", flexShrink:0 }}>
            ...
            {!isMobile && <span style={{ fontSize:"11px", color:"rgba(255,255,255,.85)", maxWidth:"120px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{authUser.name}</span>}
        ...
        <div className="brand-lockup" style={{ padding:isMobile?"12px 12px 10px":"14px 18px 12px", display:"flex", alignItems:"center", gap:"12px" }}>
          ...
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ fontSize:isMobile?"14px":"20px", fontWeight:"800", color:"#fff", lineHeight:1.2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", textShadow:"0 4px 18px rgba(0,0,0,.24)" }}>मंत्री मित्र — Mantri Mitra AI</div>
            {!isMobile && <div style={{ fontSize:"11px", color:"rgba(255,255,255,.82)", marginTop:"3px", letterSpacing:".3px" }}>AI-Powered Constituency Management System</div>}
          ...
          {!isMobile && (
            <div style={{ display:"flex", flexDirection:"column", gap:"4px", flexShrink:0, alignItems:"flex-end", padding:"8px 10px", borderRadius:"16px", background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.10)", boxShadow:"inset 0 1px 0 rgba(255,255,255,.08)" }}>
              ...
              <div style={{ fontSize:"10px", color:"rgba(255,255,255,.68)", marginTop:"3px" }}>सत्यमेव जयते</div>
            </div>
          )}
        </div>
        ...
        <div className="nav-bar desk-nav premium-panel desktop-nav-shell">
          {NAV_ITEMS.map(n=>(
            ...
            }} className={`desktop-nav-btn ${page===n.id ? "active" : ""}`}>
      ...
      <div className="app-shell" style={{ flex:1, overflowY:"auto", overflowX:"hidden", minWidth:0, WebkitOverflowScrolling:"touch" }}>
        ...
        {!isMobile && <div className="premium-panel premium-subbar" style={{ background:T.subBar, borderBottom:"1px solid "+T.subBarBorder, display:"flex", alignItems:"center" }}>
          ...
          <span style={{ fontSize:"11px", color:"var(--accent,#1B4F8A)", fontWeight:"700" }}>{NAV_LABELS[page]}</span>
        </div>}
        {/* Page heading */}
        <div style={{ padding:isMobile?"10px 12px 6px":"8px 14px 0" }}>
          <div className="premium-panel page-heading-shell" style={{ display:"flex", alignItems:"center", gap:"8px", borderBottom:"1px solid "+T.border }}>
            ...
            <div style={{ minWidth:0, flex:1 }}>
              <div style={{ fontSize:isMobile?"15px":"16px", fontWeight:"800", color:T.text, lineHeight:1.2 }}>{NAV_LABELS[page]}</div>
      ...
      <div className="mob-bottom-nav app-shell premium-panel" style={{ borderTop:"1px solid rgba(255,255,255,.12)" }}>
        {MOBILE_NAV.map(n=>(
          <button key={n.id} onClick={()=>setPage(n.id)} style={{
            ...
            color: page===n.id ? "#fff" : "var(--t-muted,#9AAAB8)",
